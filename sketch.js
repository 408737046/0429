...
function setup() {
  ...
  radioElement.style("font-size","40px")
}
...
function draw() {
  ...
  push()
    translate(width/2-capture_width/2,height/2-capture_height)
    ...
    for(var x = 0;x<captureGraphics.width;x=x+10){
      for(var y= 0;y<captureGraphics.height;y=y+span){
        ...
        if(radioElement.value()=="亮度"){
          ...
          ellipse(x,y,span*map(bk,0,255,0,1))  //透過mao指令，bk在0~255間，依照比率設定0~1間的數字
        }
